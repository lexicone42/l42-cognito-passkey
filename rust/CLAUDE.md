...
This is the **preferred Token Handler** backend — it stores JWT tokens server-side in HttpOnly session cookies and proxies auth decisions to the client-side `auth.js` library. The same `auth.js` client works against this backend or the Express backend without changes.
...
```
NOT compatible with Express's `express-session` cookie format. If migrating from Express, existing sessions will be invalidated (users re-login).
...
This backend has **no built-in rate limiting**. It assumes rate limiting is handled by the infrastructure layer:
- **AWS**: CloudFront + WAF rate-based rules, or API Gateway throttling
- **Self-hosted**: Nginx/Caddy rate limiting, or a reverse proxy like Envoy

The client-side `auth.js` has `maxLoginAttemptsBeforeDelay` for UX-level throttling, but this is trivially bypassable and not a security control. Server-side rate limiting on `/auth/session`, `/auth/refresh`, and `/auth/callback` is the deployer's responsibility.
...
| This module | Express equivalent |
|-------------|-------------------|
| `cedar/engine.rs` | `examples/backends/express/cedar-engine.js` |
| `routes/*.rs` | `examples/backends/express/server.js` (route handlers) |
